...
class VueRouter {
  ...
  init() {
    ...
    console.log("解析路由配置this.$options:", this.$options);
    this.initComponent(); // 实现两个组件
  }
  bindEvents() {
    alert('1')
    window.addEventListener("load", this.onHashChange.bind(this), alert('1'));
    window.addEventListener("hashchange", this.onHashChange.bind(this), alert('1.1'));
  }
  onHashChange() {
    // 当路径改变时 找到当前最新路径
  ...
  Vue.mixin({
    beforeCreate() {
      ...
      if (this.$options.router) {
        ...
        console.log("什么是this.$options.router:", this.$options.router);
        // 仅在根组件执行一次
